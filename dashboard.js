// dashboard.js - VERSION FINALE (Sécurisée et Anti-Crash)
    ...
    try {
        ...
        if (!dbError && patient) {
            ...
            if (welcomeMessageElement) {
                welcomeMessageElement.textContent = `Bonjour ${firstName}, content de vous revoir !`;
            }
        }
    } catch (error) {
...
async function fetchAppointments() {
    try {
        ...
        const { data: appointments, error } = await window.supabaseClient
            .from('rendez_vous')
            .select(`id, statut, motif, date_heure, type_consultation, commune, adresse_exacte, medecins ( first_name, last_name, specialite )`)
        ...
        appointments.forEach(rdv => {
            ...
            const medecinPre = rdv.medecins?.first_name || '';
            const medecinNom = rdv.medecins?.last_name || '';
            const nomComplet = medecinPre || medecinNom ? `Dr. ${medecinPre} ${medecinNom}` : 'Médecin non spécifié';
            ...
            if (rdv.statut !== 'termine') {
                ...
                actionsHTML += `<span class="text-muted small ms-2"><i class="fas fa-check-circle me-1"></i>Effectué</span>`;
            }
            ...
            const rdvCardHTML = `
                <div class="appointment-card mb-3 p-3 border rounded shadow-sm" id="rdv-${rdv.id}">
                    <div class="appointment-header d-flex justify-content-between border-bottom pb-2 mb-2">
                        ...
                        <div class="badge ${badgeClass}">${rdv.statut}</div>
                    </div>
                ...
                if (heroUpcomingContainer && heroCount < 1) {
                    heroUpcomingContainer.innerHTML = `
                        <div class="list-group-item d-flex justify-content-between align-items-center p-4">
                            ...
                            <span class="badge ${badgeClass} fs-6 px-3 py-2 rounded-pill">${rdv.statut}</span>
                        </div>
                    `;
                    heroCount++;
                }
        ...
        document.querySelectorAll('.cancel-btn').forEach(button => {
            button.addEventListener('click', async () => {
                ...
                if (confirm("Êtes-vous sûr de vouloir annuler ce rendez-vous ?")) {
                    const { error } = await window.supabaseClient.from('rendez_vous').delete().eq('id', rdvId);
                    if (!error) {
                        const el = document.getElementById(`rdv-${rdvId}`);
                        if (el) el.remove();
...
window.showRdvDetails = function(rdvId) {
    try {
        ...
        const dateMaj = dateFormatee.charAt(0).toUpperCase() + dateFormatee.slice(1);
        
        if(document.getElementById('modal-date')) document.getElementById('modal-date').textContent = `${dateMaj} à ${heureFormatee}`;

        let badgeHTML = '';
        if (rdv.statut === 'en_attente') {
            badgeHTML = '<span class="badge bg-warning text-dark fs-6 px-3 py-2 rounded-pill"><i class="fas fa-hourglass-half me-2"></i>En attente de confirmation</span>';
        } else if (rdv.statut === 'confirme') {
            badgeHTML = '<span class="badge bg-success fs-6 px-3 py-2 rounded-pill"><i class="fas fa-check me-2"></i>Confirmé par le médecin</span>';
        } else {
            badgeHTML = '<span class="badge bg-secondary fs-6 px-3 py-2 rounded-pill"><i class="fas fa-flag-checkered me-2"></i>Consultation terminée</span>';
        }
        ...
        let detailsHtml = `
            <div class="p-3 bg-light rounded border mb-3">
                ...
                <p class="mb-0 text-dark">${rdv.motif || 'Aucun motif renseigné lors de la prise de rendez-vous.'}</p>
            </div>
        `;
        ...
        if (modalEl) {
            const rdvModal = new bootstrap.Modal(modalEl);
            rdvModal.show();
        }
                ...
                <div class="card mb-3 border-0 shadow-sm border-start border-4 border-primary">
                    <div class="card-body">
                        ...
                        <button class="btn btn-sm btn-outline-danger mt-3 w-100 fw-bold" onclick="downloadOrdonnancePDF('${cons.id}')">
                            <i class="fas fa-file-pdf me-2"></i>Télécharger l'ordonnance (PDF)
                        </button>
                    </div>
                </div>
            `;
...
function downloadOrdonnancePDF(consultationId) {
    try {
...
async function fetchFacturesPreview() {
    try {
        ...
        if (summaryFactures) {
            summaryFactures.textContent = count || 0;
        }
